...
all-components: sgpsdp/sgp_time.o sgpsdp/sgp_math.o sgpsdp/sgp4sdp4.o sgpsdp/sgp_in.o sgpsdp/sgp_obs.o sgpsdp/solar.o time-tools.o predict-tools.o gtk-sat-data.o orbit-tools.o qth-data.o sat-vis.o fetch_tle.o rotor.o mongoose.o
sgpsdp/solar.o:
...
fetch_tle.o:
	gcc -c fetch_tle.c -o fetch_tle.o `pkg-config --cflags --libs glib-2.0` -lcurl
rotor.o:
	gcc -c rotor.c -o rotor.o `pkg-config --cflags --libs glib-2.0` -lcurl -lm 
mongoose.o:
	gcc -c mongoose.c -o mongoose.o `pkg-config --cflags --libs glib-2.0` -ldl -lpthread
serial.o:
	gcc -c serial.c -o serial.o `pkg-config --cflags --libs glib-2.0` -lcurl -lm
